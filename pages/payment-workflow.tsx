// ExpenseClassifier.jsx - Complete Business Classifier with Date/Month Filter + All Original Features
"use client"
...
const ExpenseClassifier = () => {
    ...
    // NEW: Date/Month Filter States
    const [dateFilterType, setDateFilterType] = useState('all'); // 'all', 'specific-date', 'specific-month', 'date-range'
    ...
    // NEW: Helper function to get available months from results
    const getAvailableMonths = () => {
        ...
        results.forEach(item => {
            if (item.date) {
                const monthYear = item.date.substring(0, 7); // YYYY-MM format
    ...
    // NEW: Helper function to get available years from results
    ...
    // NEW: Helper function to check if a date falls within the filter criteria
    ...
    // NEW: Calculate filtered summary stats
    const getFilteredSummary = () => {
        const filteredData = getFilteredResults();

        if (filteredData.length === 0) return null;

        const debits = filteredData.filter(item => item.transaction_type === 'DEBIT');
        const credits = filteredData.filter(item => item.transaction_type === 'CREDIT');

        const totalDebitAmount = debits.reduce((sum, item) => sum + (item.amount || 0), 0);
        const totalCreditAmount = credits.reduce((sum, item) => sum + (item.amount || 0), 0);
        ...
        return {
            ...
            net_balance: filteredData.net_balance_change,
            suspense_items: filteredData.filter(item => item.classification === 'SUSPENSE').length,
            high_confidence: filteredData.filter(item => item.confidence >= 95).length,
            cash_transactions: filteredData.filter(item =>
    ...
    // ENHANCED: Export to CSV with filtered data
    const exportToCSV = () => {
        try {
            const filteredData = getFilteredResults();
            ...
            } else if (dateFilterType === 'date-range' && dateRangeStart && dateRangeEnd) {
                filterSuffix = `_${dateRangeStart}_to_${dateRangeEnd}`;
            }

            a.download = `${businessCategory}_${businessSubcategory.replace(/[^a-zA-Z0-9]/g, '_')}_analysis${filterSuffix}_${new Date().toISOString().split('T')[0]}.csv`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);

            toast.success(`Exported ${filteredData.length} filtered transactions to CSV!`, {
                position: 'top-right'
            });
        } catch (error) {
    ...
    // ENHANCED: Export to Excel with filtered data
    const exportToExcel = () => {
        try {
            const filteredData = getFilteredResults();
            ...
            // Create summary sheet with filtered data
            const summaryData = [
            ...
            // Write file
            XLSX.writeFile(workbook, `${businessCategory}_${businessSubcategory.replace(/[^a-zA-Z0-9]/g, '_')}_analysis${filterSuffix}_${new Date().toISOString().split('T')[0]}.xlsx`);

            toast.success(`Exported ${filteredData.length} filtered transactions to Excel!`, {
                position: 'top-right'
            });
        } catch (error) {
    ...
    // ENHANCED: Export to Tally with filtered data
    ...
    //         if (dateFilterType === 'specific-date' && selectedDate) {
    //             filterDescription = `for date ${selectedDate}`;
    //         } else if (dateFilterType === 'specific-month' && selectedMonth) {
    //             filterDescription = `for ${formatMonthDisplay(selectedMonth)}`;
    //         } else if (dateFilterType === 'date-range' && dateRangeStart && dateRangeEnd) {
    //             filterDescription = `from ${dateRangeStart} to ${dateRangeEnd}`;
    //         } else if (filterType !== 'all') {
    //             filterDescription = `for ${filterType} transactions`;
    //         }

    //         const confirmed = window.confirm(
    //             `Export ${filteredData.length} transactions ${filterDescription} to Tally?`
    //         );

    //         if (!confirmed) return;

    //         toast.info(`Processing ${filteredData.length} filtered transactions for Tally export...`, {
    //             position: 'top-right'
    //         });

    //         const response = await startTransactionProcessing(filteredData, tallyInfo, accountDetails);

    //         toast.success(`Successfully exported ${filteredData.length} transactions to Tally!`, {
    //             position: 'top-right'
    //         });

    //         console.log("✅ Filtered ledger extraction response:", response);
    //     } catch (error) {
    //         console.error("❌ Error during filtered transaction processing:", error);
    //         toast.error(`Export failed: ${error.message}`, {
    //             position: 'top-right'
    //         });
    //     }
    ...
    // ExpenseClassifier.jsx - Updated Export to Tally Functionality
    const exportToTally = async () => {
        try {
            const filteredData = getFilteredResults();
            ...
            const dataToExport = exportType ? filteredData : results;
            ...
            toast.success(`Successfully exported ${count} transactions to Tally for ${companyName}!`, {
                position: 'top-right'
            });
    ...
    // Add this to your component JSX where the export button is located
    <button
    ...
    // ENHANCED: Filter results based on type, search term AND date filter
    const getFilteredResults = () => {
        ...
        filtered = filtered.filter(item => isDateInFilter(item.date));
    ...
    const resetForm = () => {
        ...
        setError(null);
        setFilterType('all');
        clearDateFilters(); // NEW: Clear date filters too
    };
            ...
            <div className="mx-auto px-6 py-8">
                ...
                {currentStep === 4 && results && summary && (
                    <div className="space-y-6">
                        ...
                        {/* NEW: Enhanced Date/Month Filter Section */}
                        ...
                        {/* Enhanced Summary Cards with Filtered Data */}
                                ...
                                if (!displaySummary || filteredResults.length === 0) {
                                    return (
                                        <div className="text-center py-12">
                                            ...
                                            >
                                                Clear date filter to see all transactions
                                            </button>
                                        </div>
                                    );
                                }
                                            ...
                                            <div className={`p-4 rounded-lg border ${(displaySummary.net_balance || 0) >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'}`}>
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center">
                                                        ...
                                                        <span className={`font-medium ${(displaySummary.net_balance || 0) >= 0 ? 'text-blue-700' : 'text-orange-700'}`}>Net Balance:</span>
                                                    </div>
                                                    <span className={`text-xl font-bold ${(displaySummary.net_balance || 0) >= 0 ? 'text-blue-900' : 'text-orange-900'}`}>
                                                        {(displaySummary.net_balance || 0) >= 0 ? '+' : ''}₹{(displaySummary.net_balance || displaySummary.net_balance_change)?.toLocaleString('en-IN')}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        {/* Enhanced Transaction Table with existing filters */}
                            ...
                            <div className="p-6 border-b border-gray-200">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    ...
                                    >
                                        ...
                                        Debits ({getFilteredResults().filter(item => item.transaction_type === 'DEBIT').length})
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        Credits ({getFilteredResults().filter(item => item.transaction_type === 'CREDIT').length})
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        Cash ({getFilteredResults().filter(item =>
                                            item.classification?.includes('Cash') ||
                                            item.classification?.includes('Withdrawal') ||
                                            item.classification?.includes('Deposit')
                                        ).length})
                                    ...
                                    >
                                        ...
                                        Suspense ({getFilteredResults().filter(item => item.classification === 'SUSPENSE').length})
                                    </button>
                                </div>
                            </div>

...
export default function BankFlowData() {
    ...
    // Pagination state for statements list
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage] = useState(6); // 6 cards per page for statements list

    // Pagination state for transaction details
    const [transactionPage, setTransactionPage] = useState(1);
    const [transactionsPerPage] = useState(50); // 50 transactions per page
    ...
    }, [filterType, dateFilterType, selectedDate, selectedMonthDate, dateRangeStart, dateRangeEnd, searchTerm]);
    ...
    const viewStatementDetails = (statement) => {
        ...
        setTransactionPage(1); // Reset transaction pagination
        setIsEditingAccount(false); // Reset account editing state
    };
    ...
    // Get paginated statements for list view
    const getPaginatedStatements = () => {
    ...
    const startAccountEdit = () => {
        if (selectedStatement?.header) {
            setTempAccountDetails({
                holder_name: selectedStatement.header.holder_name || '',
                ifsc_code: selectedStatement.header.ifsc_code || '',
                account_number: selectedStatement.header.account_number || ''
            });
    ...
    const startRowEdit = (index, category) => {
        // Convert from displayed index to actual filtered results index
        const actualIndex = (transactionPage - 1) * transactionsPerPage + index;
        setEditingRow(actualIndex);
        setEditingCategory(category);
        const filteredResults = getFilteredResults();
        setTempCategoryValue(filteredResults[actualIndex]?.[category] || '');
    };
    ...
    const saveRowChanges = (index, updateAll = false) => {
        const filteredResults = getFilteredResults();
        const actualIndex = (transactionPage - 1) * transactionsPerPage + index;
        const transactionToUpdate = filteredResults[actualIndex];
        ...
        const fullArrayIndex = selectedStatement.results.findIndex(
            t => t === transactionToUpdate
        );

        if (fullArrayIndex === -1) return;

        const newTransaction = {
            ...updatedTransactions[fullArrayIndex],
        ...
        if (updateAll) {
            ...
            if (vendorPrefix.length >= 6) {
                ...
                updatedTransactions[fullArrayIndex] = newTransaction;
            }
        } else {
            updatedTransactions[fullArrayIndex] = newTransaction;
        }
    ...
    const startSaveProcess = (index) => {
        const filteredResults = getFilteredResults();
        const actualIndex = (transactionPage - 1) * transactionsPerPage + index;
        const transaction = filteredResults[actualIndex];

        if (!transaction) return;

        const matchingCount = getMatchingVendorCount(actualIndex, transaction.vendor);
        if (matchingCount > 0) {
            const confirmed = window.confirm(
                `Found ${matchingCount} other transactions with similar vendor names.\n\nDo you want to update all similar transactions?`
            );
            saveRowChanges(index, confirmed);
        } else {
    ...
    // Get paginated transactions for details view
    ...
    const renderResultsStep = () => {
        ...
        const totalPages = Math.ceil(filteredResults.length / transactionsPerPage);
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
                ...
                {/* Statement Details */}
                <div className="mx-auto px-6 py-8 text-black">
                    {/* Header Information */}
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    {isEditingAccount ? (
                                        <input
                                            type="text"
                                            value={tempAccountDetails.holder_name}
                                            onChange={(e) => setTempAccountDetails({
                                                ...tempAccountDetails,
                                        ...
                                        <p className="font-medium text-gray-900">
                                            {selectedStatement.header?.ifsc_code?.match(/^[A-Za-z]+/)?.[0] || 'BANK'} – xxxx{selectedStatement.header?.account_number?.slice(-4) || 'XXXX'}
                                        </p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    {isEditingAccount ? (
                                        <input
                                            type="text"
                                            value={tempAccountDetails.account_number}
                                            onChange={(e) => setTempAccountDetails({
                                                ...tempAccountDetails,
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    {isEditingAccount ? (
                                        <input
                                            type="text"
                                            value={tempAccountDetails.ifsc_code}
                                            onChange={(e) => setTempAccountDetails({
                                                ...tempAccountDetails,
                                        ...
                                        <p className="font-medium text-gray-900">{selectedStatement.header?.ifsc_code || 'Unknown'}</p>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        {isEditingAccount && (
                            <div className="mt-4 p-3 bg-blue-50 rounded-md">
                                <p className="text-sm text-blue-700">
                                    <strong>Note:</strong> The account display name will be formatted as:{" "}
                                    {tempAccountDetails.ifsc_code?.match(/^[A-Za-z]+/)?.[0] || 'BANK'}-{tempAccountDetails.account_number || 'ACCOUNT'}
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                        <div className="px-6 py-4 border-b flex justify-between items-center">
                            ...
                            <div className="text-sm text-gray-500">
                                {filteredResults.length > transactionsPerPage && (
                                    ...
                                    {paginatedTransactions.map((transaction, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-sm max-w-xs">
                                                    {editingRow === actualIndex && editingCategory === 'classification' ? (
                                                        <select
                                                ...
                                                <td className="px-6 py-4 text-sm max-w-xs">
                                                    {editingRow === actualIndex && editingCategory === 'category' ? (
                            ...
                            <div className="px-6 py-4 bg-gray-50 border-t">
                                <div className="flex flex-col items-center space-y-4">
                                    <div className="text-sm text-gray-600">
                                        Showing {((transactionPage - 1) * transactionsPerPage) + 1} to {Math.min(transactionPage * transactionsPerPage, filteredResults.length)} of {filteredResults.length} transactions
                                    </div>
                                    <Pagination
    ...
    // Calculate pagination for statements list
    const filteredStatements = getFilteredStatements();
    const paginatedStatements = getPaginatedStatements();
    const totalStatementsPages = Math.ceil(filteredStatements.length / itemsPerPage);
            ...
            <div className="max-w-7xl mx-auto px-6 py-6">
                ...
                {filteredStatements.length > 0 && (
                    <div className="mb-6 text-center text-gray-600">
                        Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredStatements.length)} of {filteredStatements.length} bank statements
                    </div>
                )}
                ...
                {filteredStatements.length === 0 ? (
                    ...
                    <>
                        ...
                        {/* Statements List Pagination */}
                        {totalStatementsPages > 1 && (
                            <div className="flex flex-col items-center space-y-4">
                                <div className="text-sm text-gray-600">
                                    Page {currentPage} of {totalStatementsPages}
                                </div>
                                <Pagination
                                    currentPage={currentPage}
                                    totalPages={totalStatementsPages}
